...
if 'CS' in dirs:
    ...
    ax00 = {'ylabel': plotYLabels[0], 'yticks': [0, 1, 2, 3, 4, 5] }
    ax10 = {'yticks': np.arange(0,25,5)}
    nSecs = 13
    ax01 = {'ylabel': plotYLabels[1], 'yticks': np.arange(0, trackFPS*nSecs, 2*trackFPS) , 'yticklabels':  np.arange(0,nSecs,2), 'ylim':(0,trackFPS*nSecs) }
    ax11 = {'yticks': np.arange(0, trackFPS*nSecs, 2*trackFPS),'yticklabels':  np.arange(0,nSecs,2), 'ylim':(0,trackFPS*nSecs) }
    ax02 = {'ylabel': plotYLabels[2], 'yticks': [0, 1000, 2000, 3000, 4000], 'yticklabels': ['', 1, '', 3, ''] }
    ax12 = {'yticks': [0, 5000,10000, 15000, 20000], 'yticklabels': [0, '5','10', '15', ''], 'ylim':(0,21000) }
    ax03 = {'yticks': np.arange(0,10,2), 'ylabel': plotYLabels[3]  }
    ax13 = {'yticks': np.arange(0,10,2)}
    ax04 = {'ylim': (-0.2, 1.2), 'yticks': [0, 0.5, 1], 'ylabel': plotYLabels[4] }
    ax14 = {'ylim': (-0.2, 1.2), 'yticks': [0, 0.5, 1]}
    ax05 = {'xticks': [0,1,2,3,4], 'xticklabels':[1,2,3,4,5], 'xlabel': 'minutes',\
           'ylim': (1.2, -1.2), 'yticks': [-1, 0, 1], 'ylabel': plotYLabels[5] }
    ax15 = {'ylim': (1.2, -1.2), 'yticks': [-1, 0, 1], 'xticklabels': [dirs[csIndex]] }
    ...
    axLabelRot = np.arange(len(params))
    
    fig, ax = plt.subplots(nParams, 2)
    for i in xrange(len(data[0])):
            ax[i, tSeriesPlotIndex].errorbar(np.arange(len(data[:,i])), data[:,i], yerr=sem[:,i], color=colors[0], fmt='-'+markers[0])
    
    plotData = genotypeNTracks[csIndex]
    vp0 = ax[0,total5MinPlotIndex].violinplot(plotData, vPlotPosCS, showmeans=False, showmedians=True, showextrema=False, bw_method='silverman')
    bp0 = ax[0,total5MinPlotIndex].boxplot(plotData, sym='', zorder=1)
    plotScatter(ax[0,total5MinPlotIndex], plotData, scatterColor = genotypeMarker[csIndex], zOrder=2)
    
    plotData = genotypeLenTrack[csIndex]
    vp1 = ax[1,total5MinPlotIndex].violinplot(plotData, vPlotPosCS, showmeans=False, showmedians=True, showextrema=False, bw_method='silverman')
    bp1 = ax[1,total5MinPlotIndex].boxplot(plotData, sym='', zorder=1)
    plotScatter(ax[1,total5MinPlotIndex], plotData, scatterColor = genotypeMarker[csIndex], zOrder=2)
    
    plotData = genotypeDis[csIndex]
    vp2 = ax[2,total5MinPlotIndex].violinplot(plotData, vPlotPosCS, showmeans=False, showmedians=True, showextrema=False, bw_method='silverman')
    bp2 = ax[2,total5MinPlotIndex].boxplot(plotData, sym='', zorder=1)
    plotScatter(ax[2,total5MinPlotIndex], plotData, scatterColor = genotypeMarker[csIndex], zOrder=2)
    
    plotData = genotypeAvSpeed[csIndex]
    plotScatter(ax[3,total5MinPlotIndex], plotData, scatterColor = genotypeMarker[csIndex], zOrder=2)
    vp3 = ax[3,total5MinPlotIndex].violinplot(plotData, vPlotPosCS, showmeans=False, showmedians=True, showextrema=False, bw_method='silverman')
    bp3 = ax[3,total5MinPlotIndex].boxplot(plotData, sym='', zorder=1)
    
    plotData = genotypeStraight[csIndex]
    vp4 = ax[4,total5MinPlotIndex].violinplot(plotData, vPlotPosCS, showmeans=False, showmedians=True, showextrema=False, bw_method='silverman')
    bp4 = ax[4,total5MinPlotIndex].boxplot(plotData, sym='', zorder=1)
    plotScatter(ax[4,total5MinPlotIndex], plotData, scatterColor = genotypeMarker[csIndex], zOrder=2)

    plotData = genotypeGeoTacInd[csIndex]
    vp5 = ax[5,total5MinPlotIndex].violinplot(plotData, vPlotPosCS, showmeans=False, showmedians=True, showextrema=False, bw_method='silverman')
    bp5 = ax[5,total5MinPlotIndex].boxplot(plotData, sym='', zorder=1)
    plotScatter(ax[5,total5MinPlotIndex], plotData, scatterColor = genotypeMarker[csIndex], zOrder=2)
    
    # for bplot in (bp0, bp1, bp2, bp3, bp4, bp5):
    #     for pc, color in zip(bplot['boxes'], colors):
    #         pc.set_facecolor(color)
    #         pc.set_alpha(bAlpha-0.3)
    #         pc.set_edgecolor('Black')
    for vplot in (vp0, vp1, vp2, vp3, vp4, vp5):
        vplot['cmedians'].set_color('orange')
        for patch, color in zip(vplot['bodies'], colors):
            patch.set_color(color)
            patch.set_edgecolor(None)
            patch.set_alpha(bAlpha)
    
    # plt.setp([axs for axs in ax[:,total5MinPlotIndex]], xticklabels = dirs[csIndex:csIndex+1], xlabel = 'Genotype')
    # plt.setp(ax[3,total5MinPlotIndex], xticks = [1], xlabel = 'Genotype')
    # plt.setp([axs.get_xticklabels() for axs in ax[:,tSeriesPlotIndex]], rotation=45, horizontalalignment='center')
    ...
    plt.setp([axs.spines['right'].set_visible(False) for axs in ax[:,total5MinPlotIndex]])
    plt.setp([axs.spines['top'].set_visible(False) for axs in ax[:,total5MinPlotIndex]])
    plt.setp([axs.spines['right'].set_visible(False) for axs in ax[:,tSeriesPlotIndex]])
    plt.setp([axs.spines['top'].set_visible(False) for axs in ax[:,tSeriesPlotIndex]])
    plt.setp([axs.yaxis.grid(True, linestyle='-', which='major', color='lightgrey',alpha=0.5) for axs in ax[:,total5MinPlotIndex]])
    plt.setp([axs.yaxis.grid(True, linestyle='-', which='major', color='lightgrey',alpha=0.5) for axs in ax[:,tSeriesPlotIndex]])
    
    for i in xrange(len(axP)):
        for j in xrange(len(axP[i])):
            plt.setp(ax[i,j], **axP[i][j])
            for k in xrange(len(axLabelRot)):
                plt.setp(ax[axLabelRot[k], j].get_yticklabels(), rotation=90, horizontalalignment='center', verticalalignment='center')
    plt.show()
...
vp0 = ax[1,0].violinplot([da for da in genotypeNTracks], vPlotPos, showmeans=showMeans, showmedians=showMedians, showextrema=showExtrema, bw_method=bwMethod)
for x, da in enumerate(genotypeNTracks):
    ax[1,0].scatter(np.linspace(x+scatterDataWidth, x-scatterDataWidth,len(da)), da,\
                s=sSize, color = genotypeMarker[x], marker=sMarker, alpha=sAlpha, linewidths=sLinewidth, edgecolors=sEdgCol )

vp1 = ax[1,1].violinplot([da for da in genotypeLenTrack], vPlotPos, showmeans=showMeans, showmedians=showMedians, showextrema=showExtrema, bw_method=bwMethod)
for x, da in enumerate(genotypeLenTrack):
    ax[1,1].scatter(np.linspace(x+scatterDataWidth, x-scatterDataWidth,len(da)), da,\
                s=sSize, color = genotypeMarker[x], marker=sMarker, alpha=sAlpha, linewidths=sLinewidth, edgecolors=sEdgCol )

vp2 = ax[1,2].violinplot([da for da in genotypeDis], vPlotPos, showmeans=showMeans, showmedians=showMedians, showextrema=showExtrema, bw_method=bwMethod)
for x, da in enumerate(genotypeDis):
    ax[1,2].scatter(np.linspace(x+scatterDataWidth, x-scatterDataWidth,len(da)), da,\
                s=sSize, color = genotypeMarker[x], marker=sMarker, alpha=sAlpha, linewidths=sLinewidth, edgecolors=sEdgCol )

vp3 = ax[1,3].violinplot([da for da in genotypeAvSpeed], vPlotPos, showmeans=showMeans, showmedians=showMedians, showextrema=showExtrema, bw_method=bwMethod)
for x, da in enumerate(genotypeAvSpeed):
    ax[1,3].scatter(np.linspace(x+scatterDataWidth, x-scatterDataWidth,len(da)), da,\
                s=sSize, color = genotypeMarker[x], marker=sMarker, alpha=sAlpha, linewidths=sLinewidth, edgecolors=sEdgCol )

vp4 = ax[1,4].violinplot([da for da in genotypeGeoTacInd], vPlotPos, showmeans=showMeans, showmedians=showMedians, showextrema=showExtrema, bw_method=bwMethod)
for x, da in enumerate(genotypeGeoTacInd):
    ax[1,4].scatter(np.linspace(x+scatterDataWidth, x-scatterDataWidth,len(da)), da,\
                s=sSize, color = genotypeMarker[x], marker=sMarker, alpha=sAlpha, linewidths=sLinewidth, edgecolors=sEdgCol )

vp5 = ax[1,5].violinplot([da for da in genotypeStraight], vPlotPos, showmeans=showMeans, showmedians=showMedians, showextrema=showExtrema, bw_method=bwMethod)
for x, da in enumerate(genotypeStraight):
    ax[1,5].scatter(np.linspace(x+scatterDataWidth, x-scatterDataWidth,len(da)), da,\
                s=sSize, color = genotypeMarker[x], marker=sMarker, alpha=sAlpha, linewidths=sLinewidth, edgecolors=sEdgCol )
...
for vplot in (vp0, vp1, vp2, vp3, vp4, vp5):
    for patch, color in zip(vplot['bodies'], colors):
        patch.set_color(color)
        patch.set_alpha(bAlpha)
        vplot['cmedians'].set_color('violet')
...
if 'CS' in dirs:
    ...
    ax04 = {'title': plotTitlesPerUT[4], \
           'xticks': [0,1,2,3,4], 'xticklabels':[1,2,3,4,5], 'xlabel': 'minutes',\
           'ylim': (-0.2, 1.2), 'yticks': [0, 0.5, 1], 'ylabel': plotYLabels[4] }
    ax05 = {'title': plotTitlesPerUT[5], \
           'xticks': [0,1,2,3,4], 'xticklabels':[1,2,3,4,5], 'xlabel': 'minutes',\
           'ylim': (1.2, -1.2), 'yticks': [-1, -0.5, 0, 0.5, 1], 'ylabel': plotYLabels[5] }
    ...
    ax14 = {'title': plotTitles[4],\
           'ylim': (-0.2, 1.2), 'yticks': [0, 0.5, 1], 'ylabel': plotYLabels[4] }
    ax15 = {'title': plotTitles[5], 'yticks': [-1, 0, 1], 'ylabel': plotYLabels[5], 'ylim': (1.2, -1.2) }
    ...
    plotScatter(ax[1,0], plotData, scatterColor = genotypeMarker[csIndex])
    
    plotData = genotypeLenTrack[csIndex]
    bp2 = ax[1,1].boxplot(plotData, sym='', notch=True, patch_artist=True)
    plotScatter(ax[1,1], plotData, scatterColor = genotypeMarker[csIndex])
    
    plotData = genotypeDis[csIndex]
    bp1 = ax[1,2].boxplot(plotData, sym='', notch=True, patch_artist=True)
    plotScatter(ax[1,2], plotData, scatterColor = genotypeMarker[csIndex])
    
    plotData = genotypeAvSpeed[csIndex]
    plotScatter(ax[1,3], plotData, scatterColor = genotypeMarker[csIndex])
    vPlot = ax[1,3].violinplot(plotData, showmeans=False, showmedians=True, showextrema=False, bw_method='silverman')
    
    plotData = genotypeStraight[csIndex]
    bp4 = ax[1,4].boxplot(plotData, sym='', notch=True, patch_artist=True)
    plotScatter(ax[1,4], plotData, scatterColor = genotypeMarker[csIndex])

    plotData = genotypeGeoTacInd[csIndex]
    bp5 = ax[1,5].boxplot(plotData, sym='', notch=True, patch_artist=True)
    plotScatter(ax[1,5], plotData, scatterColor = genotypeMarker[csIndex])
    ...
    plotData = genotypeNTracks[csIndex]
    bp0 = ax[1,0].violinplot(plotData, vPlotPosCS, showmeans=False, showmedians=True, showextrema=False, bw_method='silverman')
    plotScatter(ax[1,0], plotData, scatterColor = genotypeMarker[csIndex])
    
    plotData = genotypeLenTrack[csIndex]
    bp2 = ax[1,1].violinplot(plotData, vPlotPosCS, showmeans=False, showmedians=True, showextrema=False, bw_method='silverman')
    plotScatter(ax[1,1], plotData, scatterColor = genotypeMarker[csIndex])
    
    plotData = genotypeDis[csIndex]
    bp1 = ax[1,2].violinplot(plotData, vPlotPosCS, showmeans=False, showmedians=True, showextrema=False, bw_method='silverman')
    plotScatter(ax[1,2], plotData, scatterColor = genotypeMarker[csIndex])
    
    plotData = genotypeAvSpeed[csIndex]
    plotScatter(ax[1,3], plotData, scatterColor = genotypeMarker[csIndex])
    vPlot = ax[1,3].violinplot(plotData, vPlotPosCS, showmeans=False, showmedians=True, showextrema=False, bw_method='silverman')
    
    plotData = genotypeGeoTacInd[csIndex]
    bp4 = ax[1,5].violinplot(plotData, vPlotPosCS, showmeans=False, showmedians=True, showextrema=False, bw_method='silverman')
    plotScatter(ax[1,5], plotData, scatterColor = genotypeMarker[csIndex])
    
    plotData = genotypeStraight[csIndex]
    bp5 = ax[1,4].violinplot(plotData, vPlotPosCS, showmeans=False, showmedians=True, showextrema=False, bw_method='silverman')
    plotScatter(ax[1,4], plotData, scatterColor = genotypeMarker[csIndex])
...
vPlot = ax[1,3].violinplot([da for da in genotypeAvSpeed], showmeans=False, showmedians=True, showextrema=False, bw_method='silverman')
for x, da in enumerate(genotypeAvSpeed):
...
fig, ax = plt.subplots(2,nParams-1, gridspec_kw = {'height_ratios':[1,2]})
fig.subplots_adjust(left=.1, bottom=.16, right=.99, top=.97, wspace=0.562, hspace = 0.500)
for c,gt in enumerate(allGenotypePerUT_Data):
    data = np.nanmean(gt[:], axis=0)
    sem = stats.sem(gt[:], axis=0)
    print sem.shape, data.shape
...
for x, da in enumerate(genotypeNTracks):
    ax[1,0].scatter(np.linspace(x+scatterDataWidth+1, x-scatterDataWidth+1,len(da)), da,\
                s=sSize, color = genotypeMarker[x], marker=sMarker, alpha=sAlpha, linewidths=sLinewidth, edgecolors=sEdgCol, zorder=2 )
