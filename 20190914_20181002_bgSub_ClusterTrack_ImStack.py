...
def getLegTipLocs(rawDir, trackParams, legContourThresh, outFname, pool):
    ...
    for i in xrange(len(croppedSubImStack)):
        if 'NoCroppedImage' not in croppedSubImStack[i][1]: #fix for the bug where 'NoCroppedImage' can eat away a frame in between properly tracked frames
    ...
    allLocs = []
    for i, im in enumerate(croppedSubIms):#find contours of each putative leg 
...
for _,rawDir in enumerate(rawDirs):
    ...
    for imdir in imdirs:
        ...
        if nFrames>nImThresh:
            ...
            if len(allLocs)>25:#minimum number of frames where legTips are detected
                try:
                    np.vstack((locs))
