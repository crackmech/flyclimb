...
#dat.to_csv('/media/aman/data/tmp.csv')        
...
#            for gType in genotypes:
#                data1 = [x[colId] for x in pooledTotalDataTmSrs[gType][tmPt]]
#                dSet.append([data1, gType, tmPt])
#            dataLen = [len(d[0]) for d in dSet]
#            lenDiff =  max(dataLen)-min(dataLen)
#            for d in dSet:
#                data1, gType, tmPt = d
#                prmValDict = extendParamDict(prmValDict, data1[:min(dataLen)], gType, tmPt)
#                dat = pd.DataFrame(prmValDict, columns=grpLbls)
#                statsFormula = robjects.Formula('result~genotype')
...
#                print "++++++++++++++++++++++++ tSeries ",colId, param, "++++++++++++++++++++++++"
#                print pdAnova['p-value']['Group']
#                print nparLD.summary_nparLD(ll1)

#                if pdAnova['p-value']['Group']<=pMin:
#                    pdWald = r_matrix_to_data_frame(ll1.rx2('Wald.test'), getLabels = True)
#                    pdAnova = r_matrix_to_data_frame(ll1.rx2('ANOVA.test'), getLabels = True)
#                    print pdWald
#                    print pdAnova
                
#                datPd = pd.DataFrame({'result':data1, 'timePoint':tmPt,
#                                      'flyNumber':[x for x in xrange(len(data1))],\
#                                      'genotype':[gType for x in xrange(len(data1))]},\
#                                      columns=grpLbls)
#
...
#nparLD(resp ~ time * group, data = rat,
...
#        df = pd.DataFrame(prmValDict, columns=grpLbls)
#        descStats = pd.DataFrame(pandas2ri.ri2py(fsa.Summarize(statsFormula, data = pandas2ri.py2ri(df))))
#        multComp = bfTrkStats.importr('multcomp')
#        nlme = bfTrkStats.importr('nlme')
#        fm1 = nlme.lme(robjects.Formula('result~timePoint*genotype'), data = pandas2ri.py2ri(df)\
#                        , random = robjects.Formula('~ 1 |flyNumber'))
#        print robjects.r['summary'](multComp.glht(fm1, linfct=multComp.mcp(timePyoint="Tukey")))
...
#                               })
#            statsFormula = robjects.Formula('result~timePoint+genotype')
#            multiComp = nparcomp.nparcomp(statsFormula, data = pandas2ri.py2ri(df), type='Tukey')
#            print multiComp
